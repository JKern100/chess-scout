...
Avoid trivial statements that are usually true for online chess (e.g., “balanced color distribution”) unless the distribution is meaningfully skewed and materially changes the interpretation.
...
let cachedSystemInstruction: { value: string; fetchedAtMs: number } | null = null;
const SYSTEM_INSTRUCTION_CACHE_MS = 60_000;

async function getEffectiveSystemInstruction(): Promise<string> {
  const now = Date.now();
  if (cachedSystemInstruction && now - cachedSystemInstruction.fetchedAtMs < SYSTEM_INSTRUCTION_CACHE_MS) {
    return cachedSystemInstruction.value;
  }
  ...
  try {
    ...
    if (error) {
      cachedSystemInstruction = { value: DEFAULT_SYSTEM_INSTRUCTION, fetchedAtMs: now };
      return DEFAULT_SYSTEM_INSTRUCTION;
    }

    const value = String((data as any)?.value ?? "").trim();
    const effective = value.length > 0 ? value : DEFAULT_SYSTEM_INSTRUCTION;
    cachedSystemInstruction = { value: effective, fetchedAtMs: now };
    return effective;
  } catch {
    cachedSystemInstruction = { value: DEFAULT_SYSTEM_INSTRUCTION, fetchedAtMs: now };
    return DEFAULT_SYSTEM_INSTRUCTION;
...
export async function generateNarrative(params: {
  ...
  try {
    ...
    const systemInstruction = await getEffectiveSystemInstruction();
    const promptHash = crypto.createHash("sha256").update(systemInstruction).digest("hex").slice(0, 12);
